...
function _renderRows(utensili){
  ...
  utensili.forEach(function(u){
    ...
    rows+='<td style="white-space:nowrap">'+
      '<a href="/utensile/'+u.id+'" class="btn-icon" title="Dettagli">â¹</a> '+
      '<a href="/utensile/'+u.id+'/modifica" class="btn-icon" title="Modifica">â</a> '+
      '<a href="/utensile/'+u.id+'/elimina" class="btn-icon btn-icon-del" title="Elimina" onclick="return _confirmDel(event,this,\x27\x27+String(u.codice_interno||\x27\x27).replace(/\x27/g,\x27\x27)+\x27\x27)">ð</a>'+
      '</td>';
    rows+='</tr>';
  });
