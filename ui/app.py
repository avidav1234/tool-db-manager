...
def debug_polyline():
    ...
    for row in rows:
        ...
        poly = row['profilo_polyline_raw']
        punti = []
        for base in range(0, len(poly)-15, 8):
            try:
                v = struct.unpack('>d', poly[base:base+8])[0]
                if not math.isnan(v) and not math.isinf(v) and 0.3 < v < 500:
                    punti.append([base, round(v, 4)])
            except: pass
        result[nome] = {'bytes': len(poly), 'valori': punti}
