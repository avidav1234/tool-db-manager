...
<tr data-search="{{ (u.codice_interno ~ ' ' ~ (u.alias or '') ~ ' ' ~ (u.descrizione or '') ~ ' ' ~ (u.nome_pinza or ''))|lower }}"
  ...
  <td style="text-align:right;background:#f0fdf4;padding-right:12px">
    ...
    {% else %}
    <span style="color:#ddd">ÃÂ¢ÃÂÃÂ</span>
    {% endif %}
  </td>
  </tr>
  ...
  <td style="font-size:12px;color:#666;max-width:110px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"
      title="{{ u.nome_pinza or '' }}">{{ u.nome_pinza or 'ÃÂ¢ÃÂÃÂ' }}</td>
...
<title>{{ titolo }} ÃÂ¢ÃÂÃÂ Tool DB Manager</title>
<style>
  ...
  <form method="POST" action="{{ action }}">
    ...
    <div class="card">
      ...
      <div class="grid">
        ...
        <div class="field"><label>Angolo punta [ÃÂÃÂ°]</label><input name="angolo_punta_gradi" type="number" step="any" value="{{ (u.get('angolo_punta_gradi') or '') | replace('None','') }}"></div>
        ...
        <div class="field"><label>Angolo elica [ÃÂÃÂ°]</label><input name="angolo_elica_gradi" type="number" step="any" value="{{ (u.get('angolo_elica_gradi') or '') | replace('None','') }}"></div>
...
def utensile_modifica(uid):
    ...
    return render_template_string(FORM_HTML,
        titolo='Modifica ÃÂ¢ÃÂÃÂ ' + row['codice_interno'], action=f'/utensile/{uid}/modifica',
...
# ---------------------------------------------------------------
# IMPORTA ÃÂ¢ÃÂÃÂ upload Excel/CSV con anteprima
# ---------------------------------------------------------------
...
<div class="grid2">
  ...
  <div>
    ...
    <div class="card">
      ...
      <div class="flash info" style="background:#e0f2fe;border-color:#0284c7;color:#0c4a6e;margin-bottom:.5rem">
        ÃÂ°ÃÂÃÂ¤ÃÂ <b>Formato rilevato:</b> {{ risultato.software }}
        {% if risultato.confidence %} &nbsp;|&nbsp; <b>Confidenza:</b> {{ "%.0f"|format(risultato.confidence*100) }}%{% endif %}
        {% if risultato.ai_usato %} &nbsp;|&nbsp; &#9889; AI mapping attivo{% else %} &nbsp;|&nbsp; ÃÂ°ÃÂÃÂÃÂ¸ Mapping euristico (nessuna API key){% endif %}
        {% if risultato.righe_totali %} &nbsp;|&nbsp; {{ risultato.righe_totali }} righe analizzate{% endif %}
      </div>
      {% endif %}
      {% if risultato.dry_run %}
      <div class="flash warn">
        Simulazione completata ÃÂ¢ÃÂÃÂ nessun dato scritto nel database.
      </div>
      {% endif %}
    </div>
    {% endif %}
  ...
  async function hmImport(){
    ...
    btn.disabled=true; st.textContent='Importazione in corsoÃÂ¢ÃÂÃÂ¦'; res.style.display='none';
    const fd=new FormData(); fd.append('file',fi.files[0]);
    try{
...
<div class="card">
  ...
  {% endif %}
  {% if risultato.dry_run %}
  <div class="flash warn" style="margin-top:1rem">Simulazione ÃÂ¢ÃÂÃÂ nessun dato scritto.</div>
  {% else %}
  ...
  <div>
    <div class="card">
      ...
      <table><tbody>
        ...
        {% if u.angolo_punta_gradi %}<tr><td style="color:#888">Angolo punta</td><td>{{ u.angolo_punta_gradi }}ÃÂÃÂ°</td></tr>{% endif %}
        {% if u.angolo_conico_gradi and u.conico %}<tr><td style="color:#888">Angolo conico</td><td>{{ u.angolo_conico_gradi }}ÃÂÃÂ°</td></tr>{% endif %}
    ...
    <div class="card">
      ...
      <table><tbody>
        ...
        <tr><td colspan="2" style="font-size:11px;color:#888;padding-top:.5rem">ÃÂ¢ÃÂÃÂ Stelo secondario ÃÂ¢ÃÂÃÂ</td></tr>
...
def salva_apikey():
    ...
    if key and 'ÃÂ¢ÃÂÃÂ¢' not in key:
        cfg = carica_config()
  ...
  <div>
    <div class="card">
      ...
      <ol style="font-size:13px;color:#555;line-height:2;padding-left:1.25rem;margin:0">
        ...
        <li>Carica il file ÃÂ¢ÃÂÃÂ mappatura automatica</li>
...
<div class="grid2">
  ...
  <div>
    ...
    <div class="card">
      <h2>ÃÂ°ÃÂÃÂ¤ÃÂ AI Parser universale</h2>
    ...
    <div class="card">
      ...
      <div style="font-size:12px;color:#555;display:flex;flex-direction:column;gap:.4rem">
        <div style="background:#f4f4f2;border-radius:5px;padding:.5rem .75rem">
          <code>bash start.sh</code> ÃÂ¢ÃÂÃÂ Avvia tutto</div>
        <div style="background:#f4f4f2;border-radius:5px;padding:.5rem .75rem">
          <code>python scheduler.py --now</code> ÃÂ¢ÃÂÃÂ Export immediato</div>
        <div style="background:#f4f4f2;border-radius:5px;padding:.5rem .75rem">
          <code>python scheduler.py --watch</code> ÃÂ¢ÃÂÃÂ Export on-change</div>
        <div style="background:#f4f4f2;border-radius:5px;padding:.5rem .75rem">
          <code>python scheduler.py --daemon</code> ÃÂ¢ÃÂÃÂ Export notturno</div>
      </div>
    </div>
  </div>
</div>
...
# ---------------------------------------------------------------
# PAGINA CAM ÃÂ¢ÃÂÃÂ stato decoder/generator per ogni CAM
# ---------------------------------------------------------------
...
<div class="card" style="margin-top:1rem;background:#f8f8f6">
  <p style="font-size:13px;color:#666;margin:0">
    ...
    Esporta un file campione dal software CAM ÃÂ¢ÃÂÃÂ caricalo nel
    <a href="http://localhost:5001" target="_blank">Format Learner</a> ÃÂ¢ÃÂÃÂ
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if azione == 'test_l2a':
                ...
                risultato = {
                    'livello': 'Test Completo L1ÃÂ¢ÃÂÃÂL4',
...
<div class="card" style="margin-bottom:1.25rem;background:#1a1a1a;color:#fff">
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:1rem;font-size:12px;text-align:center">
    <div style="background:#1d4ed8;border-radius:8px;padding:.75rem">
      ...
      <div style="font-weight:700;margin:.3rem 0">L1 ÃÂ¢ÃÂÃÂ Orchestratore</div>
    ...
    <div style="background:#166534;border-radius:8px;padding:.75rem">
      <div style="font-size:1.5rem">&#128300;</div>
      <div style="font-weight:700;margin:.3rem 0">L2a ÃÂ¢ÃÂÃÂ Struttura</div>
      <div style="color:#86efac">Haiku &#128176;</div>
      <div style="color:#bbf7d0;margin-top:.3rem">Software CAM + gruppi colonne</div>
    </div>
    <div style="background:#166534;border-radius:8px;padding:.75rem">
      <div style="font-size:1.5rem">&#128202;</div>
      <div style="font-weight:700;margin:.3rem 0">L2b ÃÂ¢ÃÂÃÂ Valori</div>
      <div style="color:#86efac">Haiku &#128176;&#128176;</div>
    ...
    <div style="background:#7c3aed;border-radius:8px;padding:.75rem">
      <div style="font-size:1.5rem">&#128270;</div>
      <div style="font-weight:700;margin:.3rem 0">L4 ÃÂ¢ÃÂÃÂ Verificatore</div>
...
<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:1rem;margin-bottom:1.25rem">
  ...
  <form method="post">
    <div class="card" style="height:100%">
      <h3 style="margin:0 0 .5rem;font-size:.95rem">&#9312; Test L2a ÃÂ¢ÃÂÃÂ Analisi Struttura</h3>
  ...
  <form method="post">
    <div class="card" style="height:100%">
      <h3 style="margin:0 0 .5rem;font-size:.95rem">&#9313; Test L2b ÃÂ¢ÃÂÃÂ Analisi Colonna</h3>
  ...
  <form method="post">
    <div class="card" style="height:100%">
      <h3 style="margin:0 0 .5rem;font-size:.95rem">&#9314; Test L3 ÃÂ¢ÃÂÃÂ Mapping</h3>
      <p style="font-size:12px;color:#666;margin:0 0 1rem">
        Haiku produce il mapping completo dopo aver analizzato tutte le colonne.
        Verifica se <code>RadiusÃÂ¢ÃÂÃÂdiametro_mm</code>, <code>GaugeÃÂ¢ÃÂÃÂfuori_pinza_mm</code>, ecc.
  ...
  <form method="post">
    <div class="card" style="border:2px solid #1d4ed8;height:100%">
      <h3 style="margin:0 0 .5rem;font-size:.95rem;color:#1d4ed8">
        &#9315; Test Completo L1ÃÂ¢ÃÂÃÂL4
      </h3>
      <p style="font-size:12px;color:#666;margin:0 0 1rem">
...
# CAM AGENT ÃÂ¢ÃÂÃÂ Agente AI per import universale CAM
# ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ

CAM_AGENT_HTML = """<!DOCTYPE html><html lang="it"><head><meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Agente CAM ÃÂ¢ÃÂÃÂ Tool DB Manager</title>
<style>
...
<div class="container">
  <div class="chat-area">
    <div class="chat-messages" id="msgs">
      <div class="msg system">&#129302; Agente CAM pronto ÃÂ¢ÃÂÃÂ carica un file o fai una domanda</div>
...
Carica un file CAM in alto e scrivi cosa vuoi fare, oppure usa i pulsanti rapidi.</div>
  ...
  <div class="side">
    ...
    <div class="pane">
      ...
      <div id="tool-log"><span style="color:#475569;font-size:.75rem">ÃÂ¢ÃÂÃÂ</span></div>
    </div>
  </div>
</div>
<script>
...
async function send(){
  ...
  try{
    // 1) Avvia job in background â ritorna subito con job_id
...
# ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ Job asincrono per l'agente CAM ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
...
# =======================================================================
# CHAT WIDGET FLOTTANTE ÃÂ¢ÃÂÃÂ iniettato in ogni pagina HTML
# =======================================================================
...
# =======================================================================
# CHAT WIDGET ÃÂ¢ÃÂÃÂ iniettato via file statico in ogni pagina HTML
# =======================================================================
...
@app.after_request
def _inject_widget(resp):
    """Inietta markup widget in ogni pagina HTML ÃÂ¢ÃÂÃÂ il JS viene da /static/widget.js."""
    if resp.content_type and 'text/html' in resp.content_type:
        html = resp.get_data(as_text=True)
        if '</body>' in html and 'ai-fab' not in html:
